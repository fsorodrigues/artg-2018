...
// importing modules
import {formatNumber,onlyUnique} from '../utils';
    ...
    let dataByYear = d3.nest()
        ...
        .map(d => {
            return {
                ...
                value: d3.sum(d.values, e => e[_accessor]),
                games: d.values
                    .filter(e => e[_accessor.replace('game_', 'info_')] != 'none')
                    .map(f => f[_accessor.replace('game_', 'info_')])
                    .toString()
                    .split(',')
                    .filter(onlyUnique)
                    .join(', ')
